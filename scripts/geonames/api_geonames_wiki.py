...
for geoname in geonames_wiki:
    ...
    data[cpt] = {'nom': nom, 'resume': resume, 'altitude' : altitude,'lat': lat, 'lng': lng, 'url': url_wikipedia,
                        'rank': rank}
    cpt+=1
...
w = shapefile.Writer('C:\\Users\Eric\\PycharmProjects\\Omeka_api\\out\\geonames_wiki.shp')
w.field('name', 'C')
...
for lieu in data:
    ...
    w.record(data[lieu]['nom'], data[lieu]['resume'], data[lieu]['altitude'], data[lieu]['url'])
