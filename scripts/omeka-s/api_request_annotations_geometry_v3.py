#!/usr/bin/env python
# coding: utf8
#repris par Benoit pour Python 3
...
nb_pages = 21
nb_item_per_page_max = 500
nb_item_per_page = 0
nb_annot = 10435

#API Key user Eric Mermet
key_identity = "TugNQNGZPUMWmVftxelRC3fykcdcEqDw"
key_credential = "9A5HSCggmtvmsPP7KhigGPrs0gxpu7CG"

# URL Omeka-s API
base_url = "http://psig.huma-num.fr/omeka-s/api/annotations?per_page="+str(nb_item_per_page_max)
#base_url = "http://psig.huma-num.fr/omeka-s/api/annotations/634695"
base_url_items = "http://psig.huma-num.fr/omeka-s/api/items/"
base_url_pages = "http://psig.huma-num.fr/omeka-s/api/annotations?per_page=" + str(nb_item_per_page_max) + "&page="
...
collection_cartes_postales_tianjin = 726645
...
base_url_items_collection3 = "http://psig.huma-num.fr/omeka-s/api/items?item_set_id=" + str(collection_cartes_postales_tianjin)
...
#Json writer
filename = dir + '\\scripts\\out\\export_annotations_for_geometries_avril2021.json'


# Pour écrire dans un fichier Shapefile
w_points = create_shapefile(dir_out, 'shape_all_geom_omeka_points_avril2021')
w_lines = create_shapefile(dir_out, 'shape_all_geom_omeka_lines_avril2021')
w_polygons = create_shapefile(dir_out, 'shape_all_geom_omeka_polygons_avril2021')
w_bbox = create_shapefile(dir_out, 'shape_all_geom_omeka_bbox_avril2021')
w_centroid = create_shapefile(dir_out, 'shape_all_geom_omeka_centroid_avril2021')
...
#for page in range(13, 14):
    ...
    base_url_pages = base_url_annot + "?per_page=" + str(nb_item_per_page_max) + "&page=" + str(page) + "&key_identity=" + key_identity + "&key_credential=" + key_credential

    x = request_api(base_url_pages)
    ...
    if page == 21:
        nb_item_per_page_max = 435

    for item in range(0, nb_item_per_page_max):
        print('id = ' + str(i))
        ...
        if 'oa:hasTarget' in x[item]:
            ...
            if 'oa:hasSelector' in x[item]['oa:hasTarget'][0]:
                ...
                if 'oa:hasSource' in x[item]['oa:hasTarget'][0]:
                    ...
                    if 'o:item_set' in x_item and 'o:media' in x_item:
